...
vector<drivingDirection> orrAgentJackie(TerrainMap &map)
{
	...
	pair<int, int> next = make_pair(0, map.getStartHex());
	...
	// create a prority queue that store and arrange nodes with increasing order in time
	...
	while (!frontier.empty())	// run until the frontier is empty
	{
		...
		directions = eval(map, next.second, finish);
		
		for (uint i = 0; i < directions.size(); ++i)
		{
			pair<int, int> nextNext = make_pair(0, map.getNeighborHex(next.second, directions[i]));
			
			if (nextNext.second != next.second)	// avoid going out of hex
			{
				// get the moving time up to the current hex
				nextNext.first = moveTime[next.second] + map.getMoveTime(next.second, directions[i]);
				
				// get the copy of the route up to the previous hex, not the current nextNext route
				route = record[next.second];
				route.push_back(directions[i]);
				
				if (isVisited[nextNext.second] == false)
				{
					isVisited[nextNext.second] = true;	// set that hex to have been visited
					
					frontier.push(make_pair(nextNext.first, nextNext.second));	// push to explore that hex later
					
					moveTime[nextNext.second] = nextNext.first;	// save the moving time
					
					record[nextNext.second] = route;	// save the direction that agent just took
				}
				else	// if that hex is already visited
				{
					// will not explore the same hex again by not putting it in the frontier
					if (nextNext.first < moveTime[nextNext.second])
					{
						moveTime[nextNext.second] = nextNext.first;	// update to the shorter route time
						
						record[nextNext.second] = route;	// update the route to that hex with the shorter one
					}
				}
			}
		}
